...
- 상어 객체와 지도 객체를 초기화합니다.
- 상어 객체의 배열을 오름차순으로 정렬합니다. (index로 바로 접근 목적)
- 상어의 최초 회전 방향, 회전 우선순위를 초기화합니다.
- 시간을 증가시키며 상어의 위치를 업데이트 합니다.
  - 상어의 회전 방향 결정, 이동 -> 지도 셀 타이머 감소 -> 중복 상어 제거 -> 시간 증가
